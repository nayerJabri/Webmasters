...
class ClientC {
    ...
    function moyenne(){
        ...
        $sql = "select
   (select count(*) from client where sexe = 'Homme') as h,
   (select count(*) from client where sexe = 'Femme') as f";
        $db = config::getConnexion();
        try{
            $res=$db->query($sql); 
            return $res->fetchAll();
    ...
    function supprimerwishlist($user){
        $sql="DELETE FROM wishlist where  userid = :user";
        $db = config::getConnexion();
                $req=$db->prepare($sql);
